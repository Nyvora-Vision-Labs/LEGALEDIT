#!/bin/bash
# Full exp3 programme: the document-grouped split (Table 9) first, then the
# random split (Table 8). Both stages are resumable -- run_exp3_grid.sh skips
# any (config, seed) already banked -- so this can be re-launched at any point.
#
# Grouped runs first because Table 9 is the table with no fallback: Table 8 has
# banked seeds to show, Table 9 has nothing until its three configs finish.
set -u
cd "$(dirname "$0")"

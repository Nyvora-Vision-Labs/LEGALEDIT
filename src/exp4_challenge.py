...
# --- prompted LLM judge -------------------------------------------------------
# Scored one probe per request, so the judge never sees the probe condition or
# the rule that generated an edit. Cached separately from exp4_raw.pkl.
import llm_judge as J
JCACHE = os.path.join(OUT, "cache", "llm_judge_exp4.npy")
if J.available() and os.environ.get("SKIP_JUDGE") != "1":
    if os.path.exists(JCACHE):
        S["LLM-judge (deepseek-chat, k=%d)" % J.K] = np.load(JCACHE)
        print("  cached llm judge")
    else:
        os.makedirs(os.path.dirname(JCACHE), exist_ok=True)
        v = J.deepseek_score(ALL, tag="exp4")
        np.save(JCACHE, v)
        S["LLM-judge (deepseek-chat, k=%d)" % J.K] = v
else:
